...
export default function Navbar() {
  ...
  return (
    ...
    <nav className={`fixed top-0 left-0 w-full z-50 bg-white border-b border-gray-100 transition-all duration-300 ${
          ...
          <div className="hidden md:flex space-x-8 items-center font-medium text-sm">
            {navLinks.map((link) => (
              <Link 
                key={link.name} 
                href={link.href} 
                className={`transition hover:text-brand-green ${isActive(link.href) ? "text-brand-green font-bold" : "text-gray-600"}`}
              >
                {link.name}
              </Link>
            ))}
            <Link href="/katalog" className="bg-brand-green text-white px-5 py-2.5 rounded-full hover:bg-emerald-900 transition shadow-lg shadow-emerald-200 flex items-center gap-2 cursor-pointer hover:-translate-y-0.5">
              <ShoppingCart size={16} /> Belanja
            </Link>
          </div>
      ...
      {/* Mobile Menu Overlay */}
      <div className={`md:hidden absolute top-full left-0 w-full bg-white border-t border-gray-100 shadow-xl transition-all duration-300 overflow-hidden ${
        ...
        <div className="px-4 py-6 space-y-2 flex flex-col">
          {navLinks.map((link) => (
            <Link 
              key={link.name} 
              href={link.href} 
              className={`block text-lg font-medium py-3 px-2 rounded-lg hover:bg-gray-50 ${isActive(link.href) ? "text-brand-green bg-green-50" : "text-gray-600"}`} 
          ...
          <div className="pt-4 mt-2 border-t border-gray-100">
            ...
            >
               <ShoppingCart size={20} /> Mulai Belanja
            </Link>
          </div>
        </div>
      </div>
